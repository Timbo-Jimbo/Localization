...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(sdf_vertex_t input)
		{
			...
			half4 effectCol = UnpackEffectColor(input.texcoord2.y, input.texcoord2.z);
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			// which UnpackEffectColor decodes as RGBA(0,0,0,0) → effectColor.a = 0 after premultiply.
