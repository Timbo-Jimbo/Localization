...
//
// Color space — two distinct paths, do NOT mix them:
//   * Vertex color (v.color) — wrap in UniTextGammaToLinearIfNeeded(v.color).
//     Canvas owns this stream and converts sRGB->linear on the CPU side; the helper handles
//     the Canvas opt-in (_UIVertexColorAlwaysGammaSpace) where the conversion is deferred
//     to the shader.
//   * UV-packed Color32 (e.g. v.texcoord2.yz from EffectPacking.PackColor) — read via
//     UniTextUnpackEffectColor(rg, ba). Canvas does NOT touch UV channels, so the bytes
//     arrive verbatim in sRGB and the gamma->linear conversion is unconditional in linear
//     color space.
...
// Decodes a Color32 packed by EffectPacking.PackColor (rg = R*256+G, ba = B*256+A)
// stored in two float UV channels. Source bytes are sRGB and Canvas does not touch
// UV channels, so the gamma->linear conversion is unconditional in linear color space.
half4 UniTextUnpackEffectColor(float rg, float ba)
{
    float r = floor(rg / 256.0);
    float g = rg - r * 256.0;
    float b = floor(ba / 256.0);
    float a = ba - b * 256.0;
    half4 c = half4(r, g, b, a) / 255.0;
    if (!IsGammaSpace())
        c.rgb = UIGammaToLinear(c.rgb);
    return c;
}
