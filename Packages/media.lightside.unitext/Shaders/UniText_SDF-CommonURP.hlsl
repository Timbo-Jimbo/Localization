...
// Source bytes are sRGB Color32 copied verbatim from C#; UV channels are not
// touched by any vertex-color pipeline, so the gamma->linear conversion is
// unconditional in linear color space.
