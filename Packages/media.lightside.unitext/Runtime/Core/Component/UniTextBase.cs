...
namespace LightSide
{
...
#endif
    {
        ...
        [SerializeField]
        [Tooltip("When enabled, the project-wide StylePreset configured in Project Settings " +
                 "is applied to this component (registered after local Styles and local " +
                 "StylePresets so they keep override priority). Disable for components that " +
                 "should opt out of global rules — e.g. debug overlays, technical text where " +
                 "markup styling would interfere.")]
        ...
        /// <summary>
        /// When <see langword="true"/> (default), <see cref="UniTextSettings.GlobalStylePreset"/>
        /// is applied to this component as an extra preset registered after the local
        /// <see cref="StylePresets"/> list. Disable to opt this component out of project-wide
        /// rules — local <see cref="Styles"/> and local <see cref="StylePresets"/> still apply.
        /// Toggling the value invalidates the parser so the next pipeline pass picks up the
        /// new effective preset set.
        /// </summary>
        public bool UseGlobalStylePreset
        {
            get => useGlobalStylePreset;
            set
            {
                if (useGlobalStylePreset == value) return;
                useGlobalStylePreset = value;
        ...
        public void SetDirty(UniTextDirtyFlags flags)
        {
            ...
            Cat.MeowFormat("[UniText] SetDirty: {0}, {1}", flags, cachedTransformData.name);
            dirtyFlags |= flags;
        ...
        private void DeinitializeAllStyles()
        {
            ...
            for (var i = 0; i < runtimeStylePresetCopies.Count; i++)
            {
                ...
                var presetStyles = config.Styles;
                for (var j = 0; j < presetStyles.Count; j++)
                {
                    presetStyles[j].DeinitializeModifier();
                }
            }
        }
        ...
        private void EnsureRuntimeConfigCopiesCreated()
        {
            ...
            if (useGlobalStylePreset)
            {
                ...
                if (global != null)
                {
                    runtimeStylePresetCopies.Add(Instantiate(global));
                }
            }
        }
        ...
        private void RegisterStylesWithParser(IReadOnlyList<Style> mods)
        {
        ...
        /// component dirty (<see cref="UniTextDirtyFlags.Text"/>) so the next pipeline pass lazily
        ...
        /// <summary>
        /// Reacts to <see cref="UniTextSettings.Changed"/> events filtered by property name.
        /// Source-side change detection in <see cref="UniTextSettings"/> guarantees this
        /// handler is invoked only when the named field actually mutated; <see cref="UniTextSettings.Affects"/>
        /// folds in the <see cref="UniTextSettings.All"/> sentinel transparently, so no per-component
        /// snapshot bookkeeping or special-case handling is needed.
        /// </summary>
        private void OnSettingsChanged(string property)
        {
            if (useGlobalStylePreset &&
                UniTextSettings.Affects(property, nameof(UniTextSettings.GlobalStylePreset)))
            {
                UnlistenStylePresetChanged();
                ListenStylePresetChanged();
                if (textProcessor != null)
            ...
            if (UniTextSettings.Affects(property, nameof(UniTextSettings.Language)) ||
                UniTextSettings.Affects(property, nameof(UniTextSettings.Dictionaries)))
            {
                SetDirty(UniTextDirtyFlags.Text);
            }
        }
        ...
        /// <summary>StylePreset instances this component is currently subscribed to (for correct unsubscription).</summary>
        private readonly List<StylePreset> subscribedConfigs = new();

        /// <summary>
        /// Subscribes <see cref="OnStylePresetChanged"/> to every <see cref="StylePreset"/>
        /// currently affecting this component — local <see cref="StylePresets"/> entries plus
        /// <see cref="UniTextSettings.GlobalStylePreset"/>. Idempotent: presets already
        ...
        internal void ListenStylePresetChanged()
        {
            ...
            {
                var config = stylePresets[i];
                if (config != null && !subscribedConfigs.Contains(config))
                {
            ...
            if (!useGlobalStylePreset) return;

            var global = UniTextSettings.GlobalStylePreset;
            if (global != null && !subscribedConfigs.Contains(global))
            {
                global.Changed += OnStylePresetChanged;
                subscribedConfigs.Add(global);
            }
        }
