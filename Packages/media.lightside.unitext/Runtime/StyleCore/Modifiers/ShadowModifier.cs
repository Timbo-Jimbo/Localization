...
namespace LightSide
{
    ...
    public class ShadowModifier : EffectModifier
    {
        ...
        /// <summary>
        /// When <see langword="true"/>, <c>dilate</c>, <c>offset</c>, and <c>softness</c> are
        /// interpreted in fixed pixels and compensated by the glyph's gradientScale so the shadow
        /// has constant on-screen geometry independent of font size. Default <see langword="false"/>
        /// — values scale with the glyph SDF padding and font metric factor.
        /// </summary>
        /// <remarks>
        /// Read on the mesh-generation hot path (<c>onGlyphEffect</c>); changing it requeues a
        /// mesh rebuild via <see cref="UniTextDirtyFlags.Color"/>.
        /// </remarks>
        ...
        protected override void OnGlyphEffect()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (fixedPixelSize)
                {
                    ...
                    meshOffX = range.offsetX;
                    meshOffY = range.offsetY;
                }
                else
                {
                    dilate = range.dilate;
