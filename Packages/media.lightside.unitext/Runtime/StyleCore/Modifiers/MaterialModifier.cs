...
namespace LightSide
{
    ...
    public class MaterialModifier : SubMeshModifier
    {
        ...
        /// <summary>
        /// Controls how the custom material composes with the base text pass on the modifier's
        /// ranges: <see cref="MaterialRenderOrder.Replace"/> suppresses the base pass (face alpha
        /// zeroed); <see cref="MaterialRenderOrder.Over"/> / <see cref="MaterialRenderOrder.Under"/>
        /// keep the base pass and layer the custom material in front of or behind it.
        /// </summary>
        /// <remarks>
        /// Read on the mesh-generation hot path (<c>OnQuadAppended</c>, <c>GetRenderOrder</c>);
        /// changing it requeues a mesh rebuild via <see cref="UniTextDirtyFlags.Color"/>. Parse
        /// and layout caches are reused — only the sub-mesh and base-face alpha get reapplied.
        /// </remarks>
        public MaterialRenderOrder RenderOrder
        {
            get => renderOrder;
            set
            {
                if (renderOrder == value) return;
                renderOrder = value;
                uniText?.SetDirty(UniTextDirtyFlags.Color);
            }
        }
        ...
        /// <summary>
        /// Stable sort key within the same <see cref="RenderOrder"/> group; lower indices render
        /// first. Use to deterministically layer multiple <see cref="MaterialModifier"/> instances
        /// with the same render order.
        /// </summary>
        ...
        /// <summary>
        /// Optional override material for emoji glyphs in the modifier's ranges. <see langword="null"/>
        /// (default) means emoji glyphs are rendered by the base emoji pass and the modifier emits
        /// no emoji sub-mesh contribution; assigning a material routes emoji quads through the
        /// modifier's sub-mesh bound to the emoji atlas.
        /// </summary>
        /// <remarks>
        /// Read in the slot-resolution path (<c>GetMaterialForSlot</c>); changing it changes which
        /// atlas/material is bound, so it requeues via <see cref="UniTextDirtyFlags.Material"/>.
        /// </remarks>
        ...
        /// <summary>
        /// Outward em-space padding applied to every sub-mesh quad so shader effects (glow, edge
        /// halos, distortion) aren't clipped by the glyph's tight bounding box. Negative reads
        /// the value from the bound material's <c>_UniTextMeshPadding</c> property (shader-driven
        /// default).
        /// </summary>
        /// <remarks>
        /// Read in <c>PrepareForParallel</c> ahead of each rebuild; changing it requeues a mesh
        /// rebuild via <see cref="UniTextDirtyFlags.Color"/>.
        /// </remarks>
        public float QuadPaddingOverride
        {
            get => quadPaddingOverride;
            set
            {
                if (Mathf.Approximately(quadPaddingOverride, value)) return;
                quadPaddingOverride = value;
                uniText?.SetDirty(UniTextDirtyFlags.Color);
            }
        }
        ...
        protected override bool ShouldIncludeCurrentGlyph()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var isEmoji = gen.font is not null && gen.font.IsColor;
                if (isEmoji && emojiMaterial is null)
                {
        ...
        protected override RenderOrder GetRenderOrder()
        {
            ...
            {
                MaterialRenderOrder.Under => LightSide.RenderOrder.Under,
                MaterialRenderOrder.Over => LightSide.RenderOrder.Over,
                MaterialRenderOrder.Replace => LightSide.RenderOrder.Base,
                _ => LightSide.RenderOrder.Over,
            };
        }
