...
namespace TimboJimboEditor.Localization
{
    ...
    public abstract class LocalizableValueDrawer : PropertyDrawer
    {
        ...
        protected virtual void DrawUnlocalizedProperty(Rect position, SerializedProperty unlocalizedProperty, string controlName)
        {
            GUI.SetNextControlName(controlName);
            EditorGUI.PropertyField(position, unlocalizedProperty, GUIContent.none, true);
        }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (localizedProperty == null || unlocalizedProperty == null)
            {
                EditorGUI.LabelField(position, label, MissingPropertiesContent);
            ...
            Rect modePosition = contentPosition;
            modePosition.xMin = modePosition.xMax - ModeButtonWidth;

            Rect valuePosition = contentPosition;
            ...
            if (assetMode)
                TryOpenLocalizedAssetPickerFromObjectSelector(valuePosition, property);

            if (assetMode)
            {
                bool previousShowMixedValue = EditorGUI.showMixedValue;
                EditorGUI.showMixedValue = activeProperty.hasMultipleDifferentValues;
                GUI.SetNextControlName(property.propertyPath);
                EditorGUI.PropertyField(valuePosition, activeProperty, GUIContent.none, true);
            ...
            else
            {
                bool previousShowMixedValue = EditorGUI.showMixedValue;
                EditorGUI.showMixedValue = activeProperty.hasMultipleDifferentValues;
                DrawUnlocalizedProperty(valuePosition, unlocalizedProperty, property.propertyPath);
                EditorGUI.showMixedValue = previousShowMixedValue;
            }
            ...
            if (GUI.Button(modePosition, modeContent, EditorStyles.miniButton))
            {
                ...
                {
                    ShowLocalizedAssetPicker(modePosition, property);
                }
            }
        ...
        private void ShowLocalizedAssetPicker(Rect buttonPosition, SerializedProperty property)
        {
            Rect activatorScreenRect = GUIUtility.GUIToScreenRect(buttonPosition);
            LocalizedAssetDropdown dropdown = new(
                LocalizedAssetType,
        ...
        private void TryOpenLocalizedAssetPickerFromObjectSelector(Rect valuePosition, SerializedProperty property)
        {
            ...
            if (currentEvent.type == EventType.KeyDown && currentEvent.keyCode == KeyCode.Space)
            {
                ...
                {
                    ShowLocalizedAssetPicker(valuePosition, property);
                    currentEvent.Use();
                    return;
                }
            }
            ...
            Rect objectPickerPosition = valuePosition;
            ...
            ShowLocalizedAssetPicker(objectPickerPosition, property);
            currentEvent.Use();
        }
        ...
        private sealed class LocalizedAssetDropdown : AdvancedDropdown
        {
            ...
            protected override void ItemSelected(AdvancedDropdownItem item)
            {
                ...
                {
                    Type assetType = _assetType;
                    UnityEngine.Object[] targets = _targets;
                    string propertyPath = _propertyPath;
                    Rect screenRect = _activatorScreenRect;
                    EditorApplication.delayCall += () =>
                        LocalizedAssetCreationWindow.Show(screenRect, assetType, targets, propertyPath);
                }
            }
            ...
            private void SelectItem(UnityEngine.Object asset)
            {
                ...
                {
                    UnityEngine.Object target = _targets[i];
                    if (target == null) continue;

                    using SerializedObject targetObject = new(target);
                    targetObject.UpdateIfRequiredOrScript();

                    SerializedProperty property = targetObject.FindProperty(_propertyPath);
                    SerializedProperty localizedProperty = property?.FindPropertyRelative(LocalizedPropertyName);
                    if (localizedProperty == null) continue;

                    localizedProperty.objectReferenceValue = asset;
    ...
    internal sealed class LocalizedAssetCreationWindow : EditorWindow
    {
        ...
        public static void Show(Rect activatorScreenRect, Type assetType, UnityEngine.Object[] targets, string propertyPath)
        {
            ...
            window.InitializeAsset($"New {assetType.Name}");
        ...
        private void InitializeAsset(string initialName)
        {
            _asset = ScriptableObject.CreateInstance(_assetType);
            _asset.name = initialName;
